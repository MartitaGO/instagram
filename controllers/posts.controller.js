...
export const insertPhoto = async (posts) => {
    ...
    const userId = await filesServices.savePhoto(posts, 500);
    // Inserta la foto usando el servicio de posts y obtiene el ID del post
    const postsId = await postsServices.insertPhoto(posts, userId);
...
export const deletePhoto = async (posts, postsId) => {
    ...
    await filesServices.deletePhoto(postsId, userId);

    // Elimina la foto del post usando el servicio de posts
    await postsServices.deletePhotoById(postsId, userId);

    await filesServices.updatePhoto(postsId, userId); // AGREGO ESTA LINEA DE USERCONTROLLER
};
...
export const listPost = async (postsId) => {
    ...
    await filesServices.listPosts(userId);
...
/* ORIGINAL // Importa el servicio que maneja las operaciones relacionadas con los archivos
import filesServices from '../services/files.services.js';
// Importa el servicio que maneja las operaciones relacionadas con los posts
import postsServices from '../services/posts.services.js';
// Importa un módulo de ayuda para manejar errores
...
export const deletePhoto = async (postsId) => {
    ...
    await filesServices.updatePhoto(postsId, post.name); // AGREGO ESTA LINEA DE USERCONTROLLER
};
